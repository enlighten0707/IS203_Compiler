...
char* generate_offset(int n, char* str) {
    int size = 0;
    char *retStr;
    while(str[size]!='\0') {
        size++;
...
void VariableDecl_class::code(ostream &s) {
  ...
  object->store_id = generate_offset(rsp_offset,RBP);
...
void Object_class::code(ostream &s) {
  ...
  if(getType()==Float) {
    if(object_table->lookup(var)->in_reg) {
      emit_movsd(object_table->lookup(var)->store_id,generate_offset(-8,RSP),s);
      emit_mov(generate_offset(-8,RSP),XMM8,s);
    } 
    else {
      emit_mov(object_table->lookup(var)->store_id,R12,s);
      emit_mov(R12,generate_offset(-8,RSP),s);
      emit_xorpd(XMM8, XMM8, s);
      emit_mov(generate_offset(-8,RSP),XMM8,s);
    }
  } 
