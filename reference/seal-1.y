...
/*  4.Variable≔ObjectID TYPEID */

     variable : OBJECTID TYPEID{ $$ = variable($1, $2); };

/*  5.CallDecl≔func object([Variable⟦,Variable⟧^* ])  TYPEID StmtBlock */
/*  nil_Variables() 空变量列表  */
/*  变量有0~多个  */
...
/*  6.StmtBlock≔{VariableDecl^*  Stmt^* }  */

    stmtBlock : '{' '}'                           { $$ = stmtBlock(nil_VariableDecls(), nil_Stmts()); }
                | '{' variableDecl_list '}'       { $$ = stmtBlock($2, nil_Stmts()); }
                | '{' stmt_list '}'               { $$ = stmtBlock(nil_VariableDecls(), $2); }
                | '{' variableDecl_list stmt_list '}' { $$ = stmtBlock($2, $3); };

/*  variableDecls => variableDecls VariableDecl | VariableDecl  */
...
/*  7.Stmt≔ ;|Expr;| IfStmt |  WhileStmt | ForStmt |  BreakStmt |  ContinueStmt | ReturnStmt |StmtBlock  */

    stmt : ';' | expr ';' | ifStmt | whileStmt | forStmt | breakStmt | continueStmt | returnStmt | stmtBlock { $$ = $1; };

/*  8.IfStmt≔if Expr StmtBlock [else StmtBlock]  */
/*  else 空的话，else的部分要用stmtBlock（nil ，nil）传  */

    ifStmt : IF expr stmtBlock { $$ = ifstmt( $2, $3, stmtBlock(nil_VariableDecls(), nil_Stmts() )); }
             | IF expr stmtBlock ELSE stmtBlock { $$ = ifstmt ( $2, $3, $5 ); };

/*  9.WhileStmt≔while Expr StmtBlock  */

    whileStmt : WHILE expr stmtBlock { $$ = whilestmt($2, $3); };

/*  10.ForStmt≔for [Expr];[Expr];[Expr]   StmtBlock  */
...
/*  14.Expr≔ ObjectID = Expr | Constant | Call | (Expr) | ObjectID 
...
/*  16.Actuals≔[Expr⟦,Expr⟧^* ]  */

    actual :        { $$ = actual(no_expr()); }
             | expr { $$ = actual($1); };
